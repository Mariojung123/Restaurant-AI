...
from models.database import InventoryLog, Recipe, SalesLog, get_db
...
@router.post("/invoice/preview", response_model=PreviewResponse)
...
class ReceiptSuggestedMatch(BaseModel):
    id: int
    name: str
    ingredient_count: int


class ReceiptPreviewItem(BaseModel):
    name: str
    quantity: int
    unit_price: Optional[float] = None
    total_price: Optional[float] = None
    suggested_match: Optional[ReceiptSuggestedMatch] = None
    match_score: float = 0.0
...
class ReceiptResultItem(BaseModel):
    name: str
    quantity: int
...
class ReceiptConfirmResponse(BaseModel):
    ...
    items_processed: int
    items_skipped: int
    items: list[ReceiptResultItem]


@router.post("/receipt/preview", response_model=ReceiptPreviewResponse)
async def preview_receipt(
    ...
    for item in parsed.items:
        ...
        if recipe:
            ...
            suggested = ReceiptSuggestedMatch(
                id=recipe.id, name=recipe.name, ingredient_count=ingredient_count
            )
        preview_items.append(
            ReceiptPreviewItem(
                name=item.name,
...
@router.post("/receipt/confirm", response_model=ReceiptConfirmResponse)
async def confirm_receipt(
    ...
    return ReceiptConfirmResponse(
        ...
        items=[ReceiptResultItem(**r) for r in results],
    )


@router.post("/invoice/confirm", response_model=InvoiceResponse)
